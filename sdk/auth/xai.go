...
func (a XAIAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	result, err := waitForXAICallback(callbackCh, opts.Prompt, state)
	if err != nil {
		return nil, err
	}
	if result.Error != "" {
...
func waitForXAICallback(callbackCh <-chan callbackResult, prompt func(prompt string) (string, error), state string) (callbackResult, error) {
	...
	for {
		...
		case <-manualPromptC:
			...
			input, errPrompt := prompt("Paste the xAI callback Token (or press Enter to keep waiting): ")
			if errPrompt != nil {
				return callbackResult{}, errPrompt
			}
			manualResult, ok, errParse := parseXAIManualCallbackToken(input, state)
			if errParse != nil {
				return callbackResult{}, errParse
			}
			if ok {
				return manualResult, nil
			}
		case <-timeoutTimer.C:
			return callbackResult{}, fmt.Errorf("xai: authentication timed out")
		}
	}
}
