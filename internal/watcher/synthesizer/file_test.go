...
func TestFileSynthesizer_Synthesize_PrefersExplicitLabelOverEmail(t *testing.T) {
	tempDir := t.TempDir()
	...
	if auths[0].Label != "GptPlus1" {
		t.Fatalf("expected explicit label to win, got %q", auths[0].Label)
	}
}
